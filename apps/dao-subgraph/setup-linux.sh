...
sudo docker compose up --no-start graph-node

# Start graph-node so we can inspect it
sudo docker compose start graph-node

# Identify the container ID
CONTAINER_ID=$(sudo docker container ls | grep graph-node | cut -d' ' -f1)

# Inspect the container to identify the host IP address
HOST_IP=$(sudo docker inspect "$CONTAINER_ID" | jq -r .[0].NetworkSettings.Networks[].Gateway)
